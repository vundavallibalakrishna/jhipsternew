...
class JhipsternewKafkaResourceIT {
    ...
    void setup() {
        ...
        JhipsternewKafkaResource kafkaResource = new JhipsternewKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jhipsternew-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipsternew-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
